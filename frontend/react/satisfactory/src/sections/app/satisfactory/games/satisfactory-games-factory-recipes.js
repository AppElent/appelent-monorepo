...
export function SatisfactoryGamesFactoryRecipes(props) {
  ...
  const setRows = (newItem) => {
    ...
    formik.setFieldValue(`factories.${factoryIndex}.recipes`, [
      ...currentRecipes,
      newItem,
    ]);
  };
  ...
  const handleCancelClick = (id) => () => {
    ...
    if (editedRow.isNew) {
      //TODO
      const oldList = formik.values.factories[factoryIndex]?.recipes.filter(
        (r) => r.id !== id
      );
  ...
  const RecipeEditInputCell = (props) => {
    ...
    let index = formik.values.factories[factoryIndex].recipes.findIndex(
      (r) => r.id === id
    );
    ...
    const currentRecipe = recipes.find(
      (recipe) =>
        recipe.className ===
        formik.values.factories[factoryIndex].recipes[index]?.recipe
    );
    ...
    // },
    {
      ...
      valueFormatter: (params) => {
        ...
        const recipename = recipes.find(
          (recipe) => recipe.className === params.value
        )?.name;
        return `${recipename}`;
      },
    },
            ...
            {recipes
              ...
              ?.ingredients.map((input) => {
                const recipe = recipes.find(
                  (r) => r.className === params.row.recipe
                );
                const itemsPerMinute = 60 / recipe.craftTime || 0;
                const total =
                  input.quantity * itemsPerMinute * params.row.amount;
                return (
